...
void loop() {
  ...
  if (Serial.available() > 0) {
	...
	if (input == "Reset_counter"){
		count =0; //reset diver-tray counter
    }
	if (input == "Tray_changed"){
      digitalWrite(RelayPauze, HIGH); //resume 
      digitalWrite(RelayHervat, LOW);
	  count = 0;
    }
  ...
  if ((digitalRead(programeurPin) == HIGH) && !sentProgrameer && !waitingForCopy) {
  ...
  if (digitalRead(programeurPin) == LOW) {
    digitalWrite(RelayProgrammeer, HIGH);
    sentProgrameer = false;
  }

  if (digitalRead(UR10InputPin2) == HIGH){
    delay(150);
